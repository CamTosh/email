<template>
  <div id="dashboard" class="inline-flex w-full flex-col items-center">
    ...
    <!-- Embed -->
    <div v-if='campaign.id' class="w-3/4 mt-8 inline-flex flex-col cursor-pointer items-center justify-center" @click='openEmbed = !openEmbed'>
    ...
    <div v-if='wantDelete' class="w-2/4 flex items-center my-4 justify-center">
      <div class="w-full bg-white py-4 rounded shadow">
        <div class="w-full inline-flex flex-col px-3 mb-2">
          ...
          <div class="w-full text-center inline-flex flex-row justify-between mt-2 px-16">
            <div class="w-16 hover:bg-gray-800 bg-gray-700 text-white text-sm py-1 px-2 border border-gray-700 rounded cursor-pointer outline-none" @click='wantDelete = !wantDelete'>
              No
            </div>
            <div class="w-16 hover:bg-red-600 bg-red-500 text-white text-sm py-1 px-2 border border-red-500 rounded cursor-pointer outline-none" @click='deleteCampaign()'>
              Yes
            </div>
          </div>
        </div>
    ...
    <div class="w-3/4 flex items-center mt-2 justify-center">
      <div class="w-full bg-white pt-4 rounded shadow">
        <div class="w-full border-b border-gray-200">
          <div class="w-full inline-flex justify-between px-3 mb-2 font-bold text-gray-700">
            <div class="px-2 mb-2">Info</div>
            <div class="hover:bg-red-600 bg-red-500 text-white text-sm py-1 px-2 border border-red-500 rounded cursor-pointer outline-none" @click='wantDelete = !wantDelete'>
              Delete
            </div>
          </div>
        </div>
        ...
        <div class="w-full py-3 inline-flex justify-around flex-col md:flex-row lg:flex-row px-8">
          ...
          <div class="md:w-3/5 lg:w-3/5 border border-gray-200 m-1 rounded h-24 bg-gray-100 mt-2 py-2 px-3 text-gray-800 py-4">
            <div class="mb-2">
              Campaign: {{ campaign.name }}              
            </div>
            <div >
              Site: <a :href='campaign.site' target="_blank" class="underline">{{ campaign.site }}</a>
            </div>
          </div>
          <div class="md:w-3/5 lg:w-3/5 border border-gray-200 m-1 rounded h-24 bg-gray-100 mt-2 py-2 px-3 py-4">
            <div class="mb-2">
              Collected Emails: {{ campaign.total }}
            </div>
            <div>
    ...
    <!-- Chart -->
    <div v-if='campaign && labels.yLabels >= 2' class="w-3/4 flex items-center mt-8 justify-center">
      <div class="w-full bg-white pt-4 rounded shadow">
       <div class="w-full">
    ...
    <div v-if='campaign.emails' class="w-3/4 flex items-center my-6 justify-center">
      <div class="w-full bg-white pt-4 rounded shadow">
        <div class="w-full border-b border-gray-200">
...
export default {
  ...
  methods: {
    async getCampaign() {
      ...
      let item = {};
      datasets.map((d) => d.dataset).forEach(function(v) {
        item[v] = (item[v] || 0) + 1;
      });
